...
| Templates         | 11 reusable mini-app definitions (`app-templates.ts`) plus 6 workflow templates (`seeds.ts`). Nothing creates fake people, cases, documents or runs. Creating a template saves a private draft owned by the signed-in principal.                                                        |
