...
// Body: { requestId, mode, profile, system, messages, tools }. Streams
// `data: {requestId, type, ...}` lines (delta | reasoning | tool-call | done |
// error | ping), the exact chunk protocol the Writer's transport already reads.
// Tool execution happens in the browser; this route only filters the tool set
// to the caller's mode and relays the model.
...
export const Route = createFileRoute("/api/writer/stream")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const { writerUser } = await import("@/lib/writer/api.server");
        ...
        } = await import("@/lib/writer/inference.server");
        ...
        if (tools.some((t) => !t || typeof t.name !== "string" || !allowed.has(t.name))) {
          return Response.json(
            { error: "This request contains an unavailable tool." },
            { status: 403 },
          );
        }
        ...
        const stream = new ReadableStream({
          async start(sink) {
            ...
            const send = (event: Record<string, unknown>) => {
              ...
              try {
                sink.enqueue(
                  encoder.encode(`data: ${JSON.stringify({ requestId, ...event })}\n\n`),
                );
              } catch {
                closed = true;
              }
            ...
            try {
              ...
              if (!aborted) console.error("[writer] stream failed:", err);
            } finally {
