// /drafts/:draftId — the Writer. The full Writer renderer (ribbon, paginated
// page, tracked changes, comments, assistant panel) mounts full-bleed inside
// the platform shell; the sidebar stays as the way back to the rest of the
// platform. The renderer's Electron surface is provided by the platform
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      adapter.installPlatformAdapter({
        draftId,
        navigateTo: (id) => {
          if (id) void navigate({ to: "/drafts/$draftId", params: { draftId: id } });
          else void navigate({ to: "/drafts" });
        },
        onDirtyChange: (dirty) => {
