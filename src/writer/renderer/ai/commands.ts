...
export interface ReplaceAllText {
  containsText: string;
  replaceText: string;
  /** Abort the entire envelope if the live match count differs. */
  ...
  for (let i = 0; i < commands.length; i++) {
    ...
    switch (name as CommandName) {
      ...
      case "replaceAllText":
        ...
        if (
          !error &&
          payload.expectedOccurrences !== undefined &&
          (!Number.isInteger(payload.expectedOccurrences) ||
            Number(payload.expectedOccurrences) < 0)
        )
          error = `${where}: expectedOccurrences must be a nonnegative integer`;
        break;
...
function runReplaceAllText(
  ...
  for (const b of blocks) {
    ...
    if (isTrackedDeleted(b.node)) {
      skippedDeleted++;
      continue;
    }
    const found = findTextMatches(b.node, b.pos, cmd.containsText, matchCase);
    replacements.push(...found);
    if (found.length) touchedIndexes.add(b.index);
  }
  if (cmd.expectedOccurrences !== undefined && replacements.length !== cmd.expectedOccurrences) {
    throw new Error(
      `Expected ${cmd.expectedOccurrences} occurrences of the exact text, found ${replacements.length}. No edits applied. Read the current text and narrow the target before retrying.`,
    );
  }
...
function runUpdateMatchedTextStyle(
  ...
  for (const b of blocks) {
    ...
    for (const match of findTextMatches(b.node, b.pos, cmd.containsText, matchCase)) {
      for (const run of match.runs) {
        ...
        if (Object.keys(markPatch).length > 0) {
          const existing = run.marks.find((m) => m.type.name === "docTextStyle")?.attrs ?? {};
          // spread keeps every non-listed attr (fontAscii/styleId/rawRPr…) alive
          const merged: Record<string, unknown> = { ...existing, ...markPatch };
          const empty = Object.values(merged).every((v) => v === null);
