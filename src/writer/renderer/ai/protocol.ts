...
export const COMMANDS_GUIDE = [
  ...
  '- "Insert a table of contents at the beginning" → commands: [{"insertToc":{"afterBlockIndex":-1}}]; if the user wants the TOC on its own page, call the insert_page_break tool with beforeBlockIndex = the first body block after the TOC',
  "- Cover page recipe: use insert_content to insert one paragraph each for title/subtitle/date at the start of the document, then updateTextStyle to enlarge the title font (e.g. {\"sizeHalfPoints\":72}, fields [\"sizeHalfPoints\"]), updateParagraphStyle to center everything and give the title {\"spaceBefore\":4800}, then call insert_page_break with beforeBlockIndex = the first body block after the cover, then view_page to confirm the cover stands alone",
  "",
  "Discipline:",
...
export const AGENT_SYSTEM_PROMPT = [
  ...
  "",
  "# Page structure",
  "- Pages are real: never separate sections with runs of empty paragraphs. Use insert_page_break (beforeBlockIndex = the block that must open the new page) so Word receives a native page break;",
  "- Always give their own page to: a cover or caption page, a table of contents, each exhibit or appendix and its heading, a signature or verification page when it follows substantive text, and a certificate of service. A multi-page brief or memo also starts its argument (the first level-1 heading after any preliminary statement) on a new page when the user asks for a formal filing;",
  "- Never leave a heading stranded at the bottom of a page with its body on the next: if view_page shows that, move the break to the heading;",
  "- Inserting or removing content can shift block indexes; re-read the block list before placing a break, and after structural edits call view_page on the affected page and fix what you see before finishing.",
  "",
  "# Comments",
