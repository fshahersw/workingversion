// Adapted from GenOffice 476e5023c9a4 (MIT); browser-native implementation.
...
export const DEFINE_STYLE_TOOL: AgentToolDef = {
  ...
  description:
    ...
    'Paragraphs using the style pick the change up automatically; direct paragraph/run formatting still overrides the style. Use the applyStyle op to put a named style on paragraphs. Existing style ids are listed by list_styles / in the tool output.',
  inputSchema: {
    type: 'object',
    properties: {
      styleId: {
...
export function resolveStyleDefinition(
  ...
  for (const k of ['basedOn', 'next'] as const) {
    ...
    if (k === 'basedOn') {
      ...
      let ancestor: AiStyleInfo | undefined = parent
      while (ancestor) {
        if (seen.has(ancestor.styleId)) return err('basedOn would create a style inheritance cycle')
        seen.add(ancestor.styleId)
        ancestor = styles.find(s => s.styleId === ancestor!.basedOn)
      }
    }
    up[k] = v
  }
