...
const DEFAULT_SETTINGS = {provider:'custom',providers:{custom:{apiKey:'',model:''}},swMode:'write',swProfile:'standard',gskToolsEnabled:false} as unknown as AiSettings
...
export function App() {
  ...
  const writeLocked = (!!writeProtection?.hash && !modifyUnlocked) || window.__officeWeb?.canWrite === false
  ...
  useEffect(() => {
    document.title = doc ? `${doc.fileName} — Seeger Weiss Writer` : 'Seeger Weiss Writer'
  }, [doc])
...
.editor-scroll .doc-page.measuring-columns { column-count: auto; width: ${colFlow.colWidthPx + twipsToPx(canvasSection?.marginLeft ?? section?.marginLeft ?? 0) + twipsToPx(canvasSection?.marginRight ?? section?.marginRight ?? 0)}px; }`}</style>
      ...
      <Ribbon
        ...
        showGrid={showGrid}
        splitView={splitView}
        trailingActions={<WriterHeader fileName={doc?.fileName} modified={hasUnsavedChanges} />}
        {...ribbonActions}
      />
