// Adapted from GenOffice 476e5023c9a4 (MIT); property-preserving native style patches.
...
export function pendingHeadingLevel(
  ...
  if (outline !== undefined && outline !== null) return outline
  // Clearing an own outline removes w:outlineLvl; the inherited level then
  // applies, exactly as parseStyles resolves the saved native package.
  if (outline !== null && cur?.headingOutlineOff) return undefined
  if (outline !== null && cur?.headingLevel && !cur.headingLevelInherited)
...
/** Patch a complete styles part without regenerating unrelated definitions. */
export function mergeStylesXml(xml: string, upserts: readonly StyleUpsert[]): string {
  for (const up of upserts) {
    let found = false
    xml = xml.replace(/<w:style\b[^>]*(?:\/>|>[\s\S]*?<\/w:style>)/g, original => {
      const id = /\bw:styleId\s*=\s*(["'])(.*?)\1/.exec(original)?.[2]
      if (id !== escapeXmlAttr(up.styleId)) return original
      if (found) throw new Error(`Duplicate style id: ${up.styleId}`)
      found = true
      return mergeStyleXml(original, up)
    })
    if (!found) {
      if (!/<\/w:styles\s*>/.test(xml)) throw new Error('The styles part has no closing w:styles element')
      xml = xml.replace(/<\/w:styles\s*>/, () => `${mergeStyleXml(null, up)}</w:styles>`)
    }
  }
  return xml
}
...
function parseTag(xml: string): { name: string; attrs: Attrs; selfClosing: boolean } {
  ...
  for (const a of m[2]!.matchAll(/([^\s=>]+)\s*=\s*(["'])(.*?)\2/g)) attrs.set(a[1]!, a[3]!.replaceAll('"', '&quot;'))
