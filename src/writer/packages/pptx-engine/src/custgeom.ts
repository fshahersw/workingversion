...
export function validCustGeomPath(p: unknown): p is CustGeomPath {
  ...
  if (!Number.isFinite(w) || !Number.isFinite(h) || w! < 1 || h! < 1 || w! > 27273042316900 || h! > 27273042316900) return false
  if (!Array.isArray(cmds) || !cmds.length || cmds.length > 10000 || cmds[0]!.op !== 'M') return false
  return cmds.every(
    (c) =>
      c &&
      typeof c === 'object' &&
      ...
      c.pts.every((v) => Number.isFinite(v) && Math.abs(v) <= 27273042316900),
  )
}
