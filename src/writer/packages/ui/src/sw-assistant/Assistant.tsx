...
export function AssistantHeader({kind,busy,onNew,onCollapse,onConnection,onLibrary}:{kind:string;busy:boolean;onNew:()=>void;onCollapse?:()=>void;onConnection:()=>void;onLibrary?:()=>void}){
 ...
 return <header className="sw-agent-header" data-sw-assistant-version="ui.2" title={`Assistant · ${kind}`}><div className="sw-agent-identity"><AssistantMark/><div><strong>Assistant</strong><small>{kind}</small></div></div><div className="sw-agent-header-actions"><button type="button" onClick={onNew} disabled={busy} aria-label="New conversation" title="New conversation"><AssistantIcon name="plus"/></button><details className="sw-agent-menu" ref={menu}><summary aria-label="Assistant options" title="Assistant options"><AssistantIcon name="more"/></summary><div className="sw-agent-menu-items">{onLibrary&&<button type="button" disabled={busy} onClick={()=>choose(onLibrary)}>Template library</button>}<button type="button" onClick={()=>choose(onConnection)}>Connection settings</button></div></details>{onCollapse&&<button type="button" onClick={onCollapse} aria-label="Collapse assistant" title="Collapse assistant"><AssistantIcon name="collapse"/></button>}</div></header>
}
