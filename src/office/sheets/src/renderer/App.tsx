...
import { modeName, publicPreferences, type WriterPreferences } from '../shared/sw-policy'
import {
  absRangeRef,
...
import { defaultTheme as greenTheme } from '@univerjs/themes'
...
export function App(): React.JSX.Element {
  ...
  if (!agentLoopRef.current) {
    ...
    agentLoopRef.current = new AgentLoop({
      ...
      skill: sheetsSkill(composeSkills('sheets+files', '', [
        ...
        // Platform build: web search through the platform, plus sandboxed
        // Python, citation checks, page reading, firm guides and the
  ...
  function isAgentConfigured(): boolean {
    // Route through main even when unconfigured: report the error, never pretend
    // the offline regex planner is a live assistant.
    return aiSettingsRef.current !== null
  }
  async function changeSwPreferences(next: WriterPreferences): Promise<void> {
    if (aiBusy || agentLoopRef.current?.busy || runStartingRef.current) return
    const updated = { ...aiSettingsRef.current, ...next } as unknown as AiSettings
    await window.desktopApi.setAiSettings(updated)
    aiSettingsRef.current=updated
  ...
  useEffect(() => {
    void window.desktopApi.getAiSettings().then(setAiSettingsState).catch(()=>setMessage('Connection settings could not be read. Local editing is available.'))
  }, [])
  ...
  async function handleSend(
    ...
    const sentAtts = swMode==='research' ? [] : overrideAttachments ?? attachmentsRef.current
    ...
    appendChat({role:'assistant',text:'The connection is not ready. Open Connection to configure the assistant. Ordinary spreadsheet editing remains available.',tools:[],isError:true})
  }
  ...
  function handleRibbonCommand(command: string): void {
    if (command === 'open') { void handleInspectWorkbook(); return }
    if (command === 'new') { void window.desktopApi.swNewWorkbook().catch(()=>setMessage('Unable to create a new workbook.')); return }
