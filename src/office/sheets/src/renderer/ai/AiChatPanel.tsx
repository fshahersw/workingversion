...
export function AiChatPanel({
  ...
  if (!isOpen) {
    return (
      <aside className="copilot collapsed">
        ...
        >
          <span className="sw-mark" aria-hidden="true">SW</span>
        </button>
      </aside>
    )
  }
  ...
  return (
    <aside
      ref={asideRef}
      className={`copilot sw-assistant${dragOver ? ' ai-panel-dragover' : ''}${resizing ? ' ai-panel-resizing' : ''}`}
    ...
    >
      ...
      />
      <AssistantHeader kind="Workbooks" busy={aiBusy} onNew={onNewChat} onCollapse={onCollapse} onConnection={()=>window.dispatchEvent(new Event('sw-open-connection'))}/>
      ...
      <div className="ai-chat" ref={chatRef} onScroll={onChatScroll}>
        ...
        {chat.length === 0 && historicChat.length === 0 && (
          <AssistantStarters app="sheets" mode={swMode} selected={scopeRange !== null} onChoose={(s)=>{onPromptChange(s);inputRef.current?.focus()}}/>
        )}
        {groupMessages(chat).map(({entry, index, key:groupKey}) => (
          <div
            key={groupKey}
          ...
          >
            {entry.role === 'user' ? (
                  ...
                  <div className="ai-msg-undelivered">
                    ...
                    {!aiBusy && (
                      <button
                        ...
                        onClick={() => {onPromptChange(entry.text);inputRef.current?.focus()}}
                      >
                        Edit request
                      </button>
                    )}
              ...
              <>
                ...
                {!entry.streaming && entry.text && <AssistantReplyActions text={entry.text}/>}
                {entry.autoApplied && (
                ...
                {entry.isError && (
                  <button
                    ...
                    onClick={() => window.dispatchEvent(new Event('sw-open-connection'))}
                  >
                    Connection
                  </button>
                )}
