...
import {publicPreferences} from '../../shared/sw-policy'
...
export function AiPanel({
  ...
  const [swPrefs,setSwPrefs]=useState(()=>publicPreferences(settings))
  const [swLibrary,setSwLibrary]=useState(false)
  const settingsRef = useRef(settings)
  settingsRef.current = {...settings,...swPrefs} as AiSettings
  useEffect(()=>{const show=()=>setSwLibrary(true);window.addEventListener('sw-open-library',show);return()=>window.removeEventListener('sw-open-library',show)},[])
  ...
  if (!open) {
    return (
      <button
        ...
        onClick={onExpand}
      >
        <span aria-hidden="true" style={{fontSize:13,fontWeight:700}}>SW</span>
      </button>
    )
  }
  ...
  return (
    <aside
      ...
      onDrop={onDrop}
    >
      <SwControls preferences={swPrefs} busy={busy} onLibrary={()=>setSwLibrary(true)} onChange={async next=>{await window.slidesApi.setAiSettings({...settingsRef.current,...next} as AiSettings);newChat();setSwPrefs(next)}}/>
      {swLibrary&&<SwLibrary onClose={()=>setSwLibrary(false)} onInsert={async deck=>{if(busy||!accessRef.current)throw new Error('Wait for the current edit to finish.');await onBeforeRunRef.current?.();await window.slidesApi.beginHistoryBatch();setBusy(true);try{await createLocalPresentation(accessRef.current,deck)}finally{await window.slidesApi.endHistoryBatch();setBusy(false)}}}/>}
      <div
        className="ai-panel-resizer"
        onPointerDown={startResize}
        role="separator"
        aria-orientation="vertical"
        aria-label="Presentation assistant"
      />
      <AssistantHeader kind="Presentations" busy={busy} onNew={newChat} onCollapse={onCollapse} onConnection={()=>window.dispatchEvent(new Event('sw-open-connection'))} onLibrary={()=>window.dispatchEvent(new Event('sw-open-library'))}/>
      ...
      <div ref={logRef} className="ai-chat" onScroll={onLogScroll}>
        ...
        {chat.length === 0 && historicChat.length === 0 && (
          <AssistantStarters app="slides" mode={swPrefs.swMode} selected={selectedIds.length > 0} onChoose={(s)=>{setInput(s);inputRef.current?.focus()}}/>
        )}
        {groupMessages(chat).map(({entry, index:i, key:groupKey}) => {
          ...
          return (
            ...
            >
              ...
              {entry.loginRequired && (
                <button className="ai-login-btn" onClick={() => window.dispatchEvent(new Event('sw-open-connection'))}>
                  Connection
                </button>
              )}
      ...
      {activeClarify ? (
        ...
        <div className="ai-composer">
          {<AssistantContext label={scopeLabel('slides',{slide:current+1,selected:selectedIds.length})} busy={busy}/>}
          ...
          <div className="ai-input-box">
            ...
            <textarea
              ...
              rows={1}
            />
            <div className="ai-input-footer"><div data-sw-composer-options />
              <button
              ...
              {/* Platform build: dictation appends to the draft; the user reviews before sending */}
              <DictationButton
                disabled={busy}
