...
export interface SlidesApi {
  swStatus(): Promise<import('./sw-policy').WriterStatus>
  swOpenConfig(): Promise<import('./sw-policy').WriterStatus>
  swTestConnection(): Promise<import('./sw-policy').WriterStatus>
...
// Platform build: `window.desktop` is declared once by the Writer (whose
// DesktopApi is a superset of DesktopFilesApi); a second declaration with a
// different type would conflict in the shared TypeScript project.
declare global {
  interface Window {
    slidesApi: SlidesApi
  }
