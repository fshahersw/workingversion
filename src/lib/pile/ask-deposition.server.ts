...
import { ANALYZE_PACK } from "./limits";
...
export async function writeDepositionAnalysis(
  input: {
    ...
    pass?: "case" | "record" | "connections" | "cross" | "cover" | "synth" | "profile" | "admissions" | "chronology" | "exhibits";
    caption?: string | null;
    citeReady?: boolean;
    witness?: string | null;
  },
  ...
  const pages = input.pages.slice(0, ANALYZE_PACK).map((p) => ({
    ...p,
  ...
  const system = [
    ...
    "When two files disagree or one omits a material admission the other contains, that is a contradiction, not a theme.",
    ...
    'Finding: {"title":string,"summary":string,"quote":string,"cite":string,"tags":string[],"value":"helpful"|"high"|"impeach"|"neutral","use":"open"|"impeach"|"notice"|"auth"|"gap"}',
    'Event: {"date":string,"title":string,"summary":string,"quote":string,"cite":string}',
    'Exhibit: {"name":string,"summary":string,"quote":string,"cite":string}',
    ...
    'Graph: {"nodes":[{"id":string,"label":string,"kind":"person"|"org"|"doc"|"theme"|"event"}],"edges":[{"from":string,"to":string,"label":string,"cite":string}]}',
    input.citeReady
  ...
  const { text: summary, model } = await converseDepAnswer({ system, user, maxTokens: 4000, signal });
...
async function converseDepAnswer(input: {
  ...
  for (const model of models) {
    try {
      ...
      lastErr = err;
      const denied =
        err instanceof BedrockError && (err.status === 400 || err.status === 403 || err.status === 404);
      if (denied) continue;
      throw err;
    }
  }
