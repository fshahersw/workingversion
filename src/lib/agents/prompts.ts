...
- matter_corpus_search: SEMANTIC search over the firm's OWN ingested corpus for one of its matters (Roundup, Zantac, Social Media, Insulin, Paraquat, GLP-1, Depo-Provera, AFFF, …) — returns the actual TEXT of the firm's collected filings ranked by meaning, and needs no case_id. Use it when the matter is one of the firm's own and the question is about what its filings SAY (a holding, expert opinion, defense, fact). Pass \`matter\` (name/MDL) + \`query\`; it lists the matters it has if it can't match. Complements the live db_* tools — for a known firm matter, reach for the corpus first for substance, and use db_* for live posture, the newest orders, or any case not in the corpus.
