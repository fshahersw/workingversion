...
export function registerSlidesIpc(): void {
  ...
  ipcMain.handle('slides:font-download', () => ({ok:false,error:'Online font downloads are disabled. Use a locally installed font.'}))
  ...
  ipcMain.handle('slides:cloud-page-generate', () => ({ok:false,error:'Use the local presentation library. External generation is disabled.'}))
...
export function startSlidesStandalone(): void {
  app.setName('Seeger Weiss Slides')
  const companionData=process.env.SW_SLIDES_USER_DATA||join(app.getPath('appData'),'SeegerWeissSlides')
  mkdirSync(companionData,{recursive:true});app.setPath('userData',companionData)
  ...
  // settings.
  // The controlled AWS transport does not use the upstream proxy discovery.
