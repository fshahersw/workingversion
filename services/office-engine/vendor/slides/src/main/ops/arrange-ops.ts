...
function resolveEndpoint(op: Op, slide: Slide, field: 'from' | 'to', index: number): SlideElement {
  ...
  if (el.transform.rot || el.transform.flipH || el.transform.flipV) {
    throw new GuidedError(`op "addConnector": "${id}" is rotated or flipped; its native connection sites are not supported by this bounded connector operation.`)
  }
  if ((el.type === 'text' || el.type === 'shape') &&
      (el.customGeometry || (el.presetGeometry && !['rect', 'roundRect', 'ellipse'].includes(el.presetGeometry)))) {
    throw new GuidedError(`op "addConnector": "${id}" has connection geometry that is not supported. Use an unrotated rectangle, rounded rectangle, ellipse, text box, picture or group.`)
  }
  if (!['text', 'shape', 'picture', 'group'].includes(el.type)) {
    throw new GuidedError(`op "addConnector": "${id}" is not a supported connectable shape.`)
  }
  return el
}
...
export function resolveConnectorWidthEmu(line: { widthPt?: unknown; widthEmu?: unknown }): number {
  ...
  if (!Number.isFinite(widthEmu) || widthEmu <= 0 || widthEmu > 20116800) {
    throw new GuidedError('op "addConnector": line width must be > 0 and at most 1584 points.')
  }
  return widthEmu
}
