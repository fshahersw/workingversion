...
function parseMs(op: Op, field: 'duration' | 'delay', fallback: number): number {
  ...
  if (typeof v !== 'number' || !Number.isFinite(v) || v < 0 || v > 2147483647) {
...
/** Do not rebuild advanced imported timing graphs from the simplified timeline. */
function commitAnimations(slide: Slide, next: SlideAnimation[]): void {
  const previous = getSlideAnimations(slide)
  const advanced = previous.some((item) => item.presetXml) || /nodeType="interactiveSeq"|<p:extLst\b|<p:tn\b/.test(slide.bodySuffix)
  if (advanced && patchSlideTimingIncrementalXml(slide.bodySuffix, next) === null) {
    throw new GuidedError('This imported animation graph cannot be changed without rewriting unmodeled timing. Nothing was changed; keep its current order or edit that advanced timeline in PowerPoint.')
  }
  setSlideAnimations(slide, next)
}
...
function parsePresetXml(op: Op, spid: number): PresetXmlInfo | undefined {
  ...
  throw new GuidedError(`op "${op.op}": raw presetXml creation is unsupported; use a documented effect. Existing imported effects are preserved.`)
}
...
function buildAnimation(op: Op, el: SlideElement): SlideAnimation {
  ...
  if (op.paragraph !== undefined) {
    ...
    if ((el.type !== 'text' && el.type !== 'shape') || !el.text?.paragraphs[op.paragraph as number]) {
      throw new GuidedError(`op "${op.op}": paragraph must identify an existing text paragraph.`)
    }
  }
...
register({
  ...
  apply(op, ctx): OpRecord {
    ...
    if (kept.length !== anims.length) commitAnimations(slide, kept)
