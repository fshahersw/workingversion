import { createEmptyWorkbookPath } from './sw-new-workbook'
import { registerSheetsControlledAiIpc } from './sw-ipc'
...
export async function createSheetsWindow(
  options: { includeAiHandlers?: boolean; initialPath?: string } = {},
  ...
  const initialPath = options.initialPath ?? forcedWorkbookPath ?? await createEmptyWorkbookPath(join(app.getPath('userData'),'working'))
  queueWorkbookForView(window.webContents, initialPath)
...
export function registerSheetsIpc(): void {
  ...
  ipcMain.handle('sw:new-workbook', async (event) => {
    sessionFor(event)
    if (event.senderFrame !== event.sender.mainFrame) throw new Error('Untrusted workbook frame.')
    const initialPath=await createEmptyWorkbookPath(join(app.getPath('userData'),'working'))
    await createSheetsWindow({initialPath})
  })
...
export function registerSheetsAiIpc(): void {
  ...
  registerSheetsControlledAiIpc(() => runtime.rendererFile, () => runtime.rendererUrl)
}
...
export function startSheetsStandalone(): void {
  app.setName('Seeger Weiss Sheets')
  app.setPath('userData', join(app.getPath('appData'), 'Seeger Weiss Sheets'))
  if (process.platform === 'win32') app.setAppUserModelId('com.seegerweiss.sheets.preview')
  ...
  // Controlled connection does not use the upstream cloud proxy.
  app.whenReady().then(() => {
...
/** Browser migration host: document-bound native engine session, no windows or AI handlers. */
export function registerSheetsWebHost(contents: WebContents, path: string, binary: string): void {
 configureSheetsRuntime({preloadPath:"",rendererFile:"",sidecarPath:binary});
 const client=new XlsxSidecarClient(binary);sidecar=client;registerSheetsSession(contents,client);registerSheetsIpc();queueWorkbookForView(contents,path);
}
