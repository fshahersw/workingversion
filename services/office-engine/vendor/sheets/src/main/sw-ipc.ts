...
export function registerSheetsControlledAiIpc(rendererFile:()=>string, rendererUrl:()=>string|undefined):void{
  ...
  function trust(e:IpcMainInvokeEvent){
    ...
    if(e.senderFrame!==e.sender.mainFrame)throw new Error('This operation is only available to the workbook window.')
  ...
  const prefsPath=()=>join(app.getPath('userData'),'sheets-preferences.json')
  ...
  function status():WriterStatus{
    ...
    try{const c=loadConnection(app.getPath('userData'));let configured=false;try{configuredProfile(c,'standard');configured=true}catch{}
      return{configured,researchConfigured:!!c.research?.gatewayUrl&&!!c.research?.toolName,configPath,
  ...
  ipcMain.handle('sw:test',async(e)=>{
    ...
    try{const c=loadConnection(app.getPath('userData'));testedKey=JSON.stringify(c);let text=''
      ...
      tested.set(JSON.stringify(c),new Date().toISOString());return{...status(),message:'Connection tested successfully. No workbook content was sent.'}
    }catch(err){if(testedKey)tested.delete(testedKey);return{...status(),message:writerError(err)}}finally{clearTimeout(timer)}
  })
  ipcMain.handle('sw:approve-research',(e,value)=>{trust(e);const q=publicQuery(value);research.set(e.sender.id,{query:q,expires:Date.now()+180000});return q})
  ...
  ipcMain.handle('ai:stream',async(e,request:AiStreamRequest)=>{
    ...
    try{
      ...
      if(mode==='research'&&(!research.has(e.sender.id)||research.get(e.sender.id)!.expires<Date.now()))throw new Error('Approve a public question in the Research tab first.')
      ...
      if(mode==='research'){
        const approved=research.get(e.sender.id)!
        if(!approved.result)approved.result=searchPublic(c,approved.query,controller.signal)
        const results=await approved.result
        payload=publicResearchRequest(approved.query,results)
      }
    ...
    }catch(error){if(timedOut)send({type:'error',error:'The request exceeded the time limit. Completed workbook changes can be restored from the response.',errorCode:'timeout'});else if(controller.signal.aborted)send({type:'done'});else send({type:'error',error:writerError(error)})}
  ...
  ipcMain.handle('ai:web-search',async(e)=>{
    trust(e);const approved=research.get(e.sender.id)
    if(!approved||approved.expires<Date.now())return{results:[],method:'error',error:'Enter and send a public question in the Research tab. Document text is not used as a search query.'}
    try{
      if(!approved.result){const timeout=AbortSignal.timeout(45000);approved.result=searchPublic(loadConnection(app.getPath('userData')),approved.query,timeout)}
      return await approved.result
    }catch{return{results:[],method:'error',error:'The approved research service could not complete this search. There is no external fallback.'}}
  })
  ...
  ipcMain.handle('sheets:ai-generate-image',(e)=>{trust(e);return{error:'Remote image generation is disabled in this document companion.'}})
}
